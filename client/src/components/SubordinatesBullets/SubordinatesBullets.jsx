...
export default function SubordinatesBullets() {
  ...
  const { subordinateInfo, setSubordinateInfo } =
    useContext(SubordinateContext);
  const navigate = useNavigate();
  const [certainAward, setCertainAward] = useLocalStorage("certainAward");
  const [certainSubordinateID, setCertainSubordinateID] = useLocalStorage(
    "certainSubordinateID"
  );
  ...
  const fetchBullets = () => {
    fetch(
      `http://localhost:3001/bullet/completed/${certainSubordinateID}/${certainAward}`
    )
      .then((res) => {
        if (!res.ok) {
  ...
  useEffect(() => {
    if (
      !subordinateInfo ||
      (Array.isArray(subordinateInfo) && subordinateInfo.length === 0)
    ) {
  ...
  return (
    <>
      ...
      <div className="subordinates-bullets-page-container">
        <div
          className={!revisedHidden ? "subordinate-bullet-card" : ""}
          hidden={revisedHidden}
        >
        ...
        {!subordinateInfo ||
        (Array.isArray(subordinateInfo) && subordinateInfo.length === 0) ? (
          <p className="no-bullets-message">
            No completed bullets for this subordinate
          </p>
        ) : (
          subordinateInfo.map((bu, i) => (
            <div key={i} className="subordinate-bullet-card-ethan">
              ...
              <p className="subordinate-bullet-section">
                <span
                  className="subordinate-bullet-label"
                  style={{ color: "#B67EF0" }}
                >
                  {bu.status}
                </span>{" "}
                ...
                <span
                  ...
                  style={{ color: "#B67EF0" }}
                >
                  {new Date(bu.updated_at).toLocaleDateString()} -{" "}
                  {new Date(bu.updated_at).toLocaleTimeString()}
                </span>{" "}
              </p>
              <p className="subordinate-bullet-section">
